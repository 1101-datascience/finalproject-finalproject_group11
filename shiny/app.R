...
ui <- fluidPage(
    shinyUI(navbarPage("Final Project",theme = shinytheme("cosmo"),
        ...
        navbarMenu("Data Processing", icon = icon("far fa-angry"),
            tabPanel("I Distrubution",fluid = TRUE,
                sidebarLayout(
                    sidebarPanel(
            ...
            tabPanel("III feature",fluid = TRUE,
                     sidebarLayout(
                       sidebarPanel(
        ...
        navbarMenu("Training", icon = icon("far fa-angry"),
                   tabPanel("Impoertance",fluid = TRUE,
                            sidebarLayout(
                              sidebarPanel(
                                p("feature's importance from random forest")
                              ),
                              
                              mainPanel(
                                imageOutput("imp",width = "40px", height = "30px")
                              )
                            )
                   ),
        ...
        navbarMenu("Kaggle", icon = icon("far fa-angry"),
                   tabPanel("About", fluid = TRUE,
                            h2(p("About our Project")),
                            img(src = "bost.png", height = 240, width = 800),
                            img(src = "kaggle_submit.png", height = 200, width = 800),
                   )
...
server <- function(input, output) {
    ...
    CorMat <- cor(df[ ,c("crim","zn","indus","chas","nox","rm","age","dis","rad","tax","ptratio","black","lstat","medv")])
    ...
    output$mod <- renderImage({
      
      if (input$md == 1){
        ss = "KNN_model.png"
      }else if(input$md == 2){
        ss = "LinearRegression_model.png"
      }else if(input$md == 3){
        ss = "RandomForest_model.png"
      }
